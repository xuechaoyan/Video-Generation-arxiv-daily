"""Read must-read papers with Cursor and write Chinese full-paper summaries."""
...
def select_papers_for_summary(
    ...
    """Prefer must-read papers, then fill remaining slots by existing order."""
    must_read = [paper for paper in papers if paper.get("priority") == "must-read"]
    others = [paper for paper in papers if paper.get("priority") != "must-read"]
    return (must_read + others)[:max_papers]
...
def write_digest(
    ...
    digest_path.with_suffix(".md").write_text(markdown, encoding="utf-8")
    output_dir = digest_path.parent
    ...
    if generated_at:
        ...
        (output_dir / f"{date_name}.md").write_text(markdown, encoding="utf-8")
        (output_dir / f"{date_name}.json").write_text(
            json.dumps(payload, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
...
def main() -> None:
    ...
    papers = payload.get("papers", [])
    selected = select_papers_for_summary(papers, args.max_papers)
